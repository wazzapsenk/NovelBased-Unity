...
{
    public class SetRoleNode : INode
    {
        protected override string DOCUMENTATION => "https://www.wlabsocks.com/wiki/index.php/Set_Role_Node";
        ...
        public override void Draw()
        {
            ...
            var roleDict = GraphWindow.CurrentDatabase.nodeDataList.OfType<RoleData>()
                .Select(a => new {a.ID, a.RoleName}).ToDictionary(d => d.ID, d => d.RoleName);
            ...
            roleList.choices.Insert(0, "- Dismiss Role");

            var index = roleDict.Keys.ToList().IndexOf(RoleID);
            roleList.index = index == -1 ? 0 : index + 1;

            if (roleList.index == 0) RoleID = string.Empty;
            ...
            roleList.AddClasses("ide-node__set-role-dropdown-field");
