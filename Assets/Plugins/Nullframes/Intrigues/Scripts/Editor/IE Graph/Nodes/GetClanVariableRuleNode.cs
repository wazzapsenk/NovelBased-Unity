...
{
    public class GetClanVariableRuleNode : INode
    {
        protected override string DOCUMENTATION => "https://www.wlabsocks.com/wiki/index.php/Get_Clan_Variable_Rule_Node";
        ...
        public override void Draw()
        {
            ...
            var titleLabel = IEGraphUtility.CreateLabel("<color=#CCC377>\u25CF</color> Get [Clan] Variable");
            ...
            void loadVariables()
            {
                variables = GraphWindow.CurrentDatabase.variablePool.Where(v => v.Type != NType.Object).Select(s => new { s.id, s.name })
        ...
        private void LoadPorts()
        {
            ...
            if (outputBox.Children().LastOrDefault() is not Port port) return;
            
            port.portColor = STATIC.RedPort;
            port.GetChild<Label>().style.unityFontStyleAndWeight = new StyleEnum<FontStyle>(FontStyle.Italic);
        }
    }
}