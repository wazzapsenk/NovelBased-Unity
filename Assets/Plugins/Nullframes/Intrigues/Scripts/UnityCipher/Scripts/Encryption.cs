...
namespace Nullframes.Intrigues.SaveSystem
{
    public class Encryption
    {
        ...
        {
            if (!IsBase64String(encrypted)) return encrypted;
            byte[] decripted = Decrypt(Convert.FromBase64String(encrypted), password);
        ...
        /// </summary>
        public static byte[] Decrypt(IEnumerable<byte> src, string password)
        {
            RijndaelManaged rij = SetupRijndaelManaged;
