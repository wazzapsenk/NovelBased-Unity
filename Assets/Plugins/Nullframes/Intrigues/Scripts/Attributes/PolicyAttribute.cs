...
namespace Nullframes.Intrigues.Attributes
{
    ...
    public class PolicyAttribute : PropertyAttribute { }

#if UNITY_EDITOR
    [CustomPropertyDrawer(typeof(PolicyAttribute))]
    public class PolicyDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            if (IM.Exists && IM.DatabaseExists)
            {
                var list = IM.IEDatabase.policyCatalog.OrderBy(c => c.PolicyName)
                    .Select(g => new { g.ID, g.PolicyName }).ToDictionary(c => c.ID, c => c.PolicyName);
