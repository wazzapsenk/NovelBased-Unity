...
{
    public class FamilyAttribute : PropertyAttribute { }

#if UNITY_EDITOR
    [CustomPropertyDrawer(typeof(FamilyAttribute))]
    public class FamilyDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            if (IM.Exists && IM.DatabaseExists)
            {
                var list = IM.IEDatabase.groupDataList.Where(g => g is FamilyGroupData).OfType<FamilyGroupData>().OrderBy(c => c.Title)
                    .Select(g => new { g.ID, g.Title }).ToDictionary(c => c.ID, c => c.Title);
