...
namespace Nullframes.Intrigues.Attributes
{
    ...
    public class ClanAttribute : PropertyAttribute { }

#if UNITY_EDITOR
    [CustomPropertyDrawer(typeof(ClanAttribute))]
    public class ClanDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            if (IM.Exists && IM.DatabaseExists)
            {
                var list = IM.IEDatabase.groupDataList.Where(g => g is ClanGroupData).OfType<ClanGroupData>().OrderBy(c => c.Title)
